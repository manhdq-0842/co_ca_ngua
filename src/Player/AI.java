...
public class AI extends Player {
	...
	public AI(int ID, int diemXP, int diemdung, Horse[] horses,Player[] players, Trai[] trais) {
	...
	@Override//ghi de
	public int go(int X, int Y, int soBuoc, int turn) {//thuc hien cho con ngua di tren ban co
		if(winner() == 1) {
		...
		if (soBuoc != 0) {
				...
				for (Horse h: getHorses()) {
					if (h.getPosition() == -1) {
						...
						if (X == h.getX() && Y == h.getY()) {
							if(circles[i].getDanhdau() == 10) {//neu chua co ngua moi duoc xuat quan
			...
			for (Horse h: getHorses()) {//vong lap chay tu con ngua dau tien trong mang horse,kieu du lieu
										//Horse cho bien h,gan buoc di cho tung con ngua cho trong mang
				...
				if ((h.getX() == X && h.getY() == Y && h.getWinner() == - 1)) {
					...
					if(((pos + soBuoc) == 1 || (pos + soBuoc == 15) || (pos + soBuoc) == 29 || (pos + soBuoc) == 43) && (h.getSteped() + soBuoc < 55)) {
						...
						//da ngua
						if((circles[pos + soBuoc].getDanhdau()) != 10 && (h.getSteped() + soBuoc <= 55)) {//kiem tra dong doi
							if((circles[pos + soBuoc].getDanhdau()) != turn){//se da ngua
							int luot = circles[pos+ soBuoc].getDanhdau();
							circles[pos+ soBuoc].setDanhdau(-1);//con ngua tai vi tri circles =-1 se bi da,10 la ko co ngua
																//0,1,2,3 tuong ung ngua cac chuong						
							return  luot + 10;					//tra ve nguoi choi bi da'
							}
					...
					//neu tong so buoc > 55 thi ko thuc hien gi het
					if(h.getSteped() == 55) {
						circles[pos].setDanhdau(10);
						vedich(h, turn, soBuoc,pos);
						return 1;
	...
	public int tienquan(int X, int Y, int soBuoc, int turn) {
		for (Horse h: getHorses()) {
			...
			if (circles[pos].getX() == X && circles[pos].getY() == Y) {
				if(pos + soBuoc >55) {//di qua vi tri 0.
					pos = pos - 56;
				}
